...
export class HTTP$RESPONSE$CODES$GroupChartHelper implements ChartHelper {
			...
			if (groups.hasOwnProperty(indx)) {
				...
				for (const i in dataList) {
					if (dataList.hasOwnProperty(i)) {
						...
						let value = 0;
						if (dataMap['400'] !== undefined) {
							value += dataMap['400'];
						}
						if (dataMap['500'] !== undefined) {
							value += dataMap['500'];
						}
						if (dataMap['600'] !== undefined) {
							value += dataMap['600'];
						} 
						
						series.push({ 'unit': unit, 'time': data.timestamp, 'value': ChartDataHelper.formatNumericValue(value, unit) });
